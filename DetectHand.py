    ...
    def findFingers(self, frame, draw=True):
        ...
        if self.results.multi_hand_landmarks and self.results.multi_handedness:
            for idx, handLms in enumerate(self.results.multi_hand_landmarks):
                # Get hand label
                handType = self.results.multi_handedness[idx].classification[0].label  # 'Left' or 'Right'

                if draw:
                    # Draw hand landmarks
                    self.mpDraw.draw_landmarks(frame, handLms, self.handsMp.HAND_CONNECTIONS)

                    # Find the bounding box (optional but nice)
                    xList = []
                    yList = []
                    ...
                    if xList and yList:
                        ...
                        # Put the label
                        cv2.putText(frame, handType, (xmin-30, ymin-30),
